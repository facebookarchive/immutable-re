...
let module TransientVectorImpl = {
  type t 'a = {
  ...
  let addFirst
    ...
    if ((tailIsFull leftCount) && (tailIsNotEmpty rightCount)) {
      ...
      transientVec.middle = IndexedTrie.Impl.addFirstLeaf
  ...
  let addLast
    ...
    if ((tailIsNotFull leftCount) && (tailIsEmpty rightCount)) {
      ...
      transientVec.middle = IndexedTrie.Impl.addLastLeaf
        IndexedTrie.Mutator.updateLevelTransient
        owner
        right
        middle;
...
let mutate ({ left, middle, right }: PersistentVector.t 'a): (t 'a) => Transient.create ({
  left: if (CopyOnWriteArray.count left > 0) (TransientVectorImpl.tailCopyAndExpand left) else [||],
  leftCount: CopyOnWriteArray.count left,
  middle,
  right: if (CopyOnWriteArray.count right > 0) (TransientVectorImpl.tailCopyAndExpand right) else [||],
...
let persist (transient: t 'a): (PersistentVector.t 'a) => {
  let {
    ...
    right,
    rightCount,
  }: TransientVectorImpl.t 'a = transient |> Transient.persist;
...
let updateAllImpl
    ...
    ({
      ...
      right,
      rightCount
    } as transientVec: TransientVectorImpl.t 'a): (TransientVectorImpl.t 'a) => {
  let index = ref 0;
  let updater value => {
  ...
  let middle = middle |> IndexedTrie.Impl.updateAll
    IndexedTrie.Mutator.updateLevelTransient
    IndexedTrie.Mutator.updateLeafTransient
    owner
    updater;
