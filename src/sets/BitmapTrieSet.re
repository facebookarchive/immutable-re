...
let rec add =
  ...
  | Level(bitmap, nodes, _) =>
    ...
    if (BitmapTrie.containsNode(bitmap, bit)) {
      ...
      let newChildNode =
        childNode |> add(comparator, updateLevelNode, owner, depth + 1, hash, value);
  ...
  | Collision(entryHash, entrySet) when hash === entryHash =>
    ...
    if (newEntrySet === entrySet) {
      ...
      Collision(entryHash, newEntrySet)
    }
...
let rec contains =
        (comparator: Comparator.t('a), depth: int, hash: int, value: 'a, set: t('a))
        : bool =>
  switch set {
  | Level(bitmap, nodes, _) =>
    let bit = BitmapTrie.bitPos(hash, depth);
...
let reduce =
    (~while_ as predicate: ('acc, 'a) => bool, f: ('acc, 'a) => 'acc, acc: 'acc, map: t('a))
    : 'acc =>
...
let rec remove =
  ...
  | Level(bitmap, nodes, _) =>
    ...
    if (BitmapTrie.containsNode(bitmap, bit)) {
      let childNode = nodes[index];
      let newChildNode =
        childNode |> remove(comparator, updateLevelNode, owner, depth + 1, hash, value);
  ...
  | Entry(entryHash, entryValue)
      when hash === entryHash && Comparator.toEquality(comparator, entryValue, value) =>
    Empty
  | _ => set
  };
...
let updateLevelNodePersistent =
    (_: Transient.Owner.t, index: int, childNode: t('a), Level(bitmap, nodes, _): t('a))
    : t('a) =>
...
let updateLevelNodeTransient =
    (
      ...
      index: int,
      childNode: t('a),
      Level(bitmap, nodes, nodeOwner) as node: t('a)
    )
    : t('a) =>
  if (nodeOwner === owner) {
