...
module MakeGeneric =
  ...
  include Base;
  include (
    TransientCollection.MakeGeneric(Base):
      TransientCollection.SGeneric with type t('a) := t('a) and type elt('a) := elt('a)
  );
  let addAll = (iter: Iterable.t(elt('a)), transient: t('a)) : t('a) =>
    iter
    |> Iterable.reduce(~while_=Functions.alwaysTrue2, (acc, next) => acc |> add(next), transient);
};
