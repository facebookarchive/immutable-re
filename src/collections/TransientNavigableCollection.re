...
module type SGeneric = {
  ...
  include TransientSequentialCollection.SGeneric with type t 'a := t 'a and type elt 'a := elt 'a;
...
let module MakeGeneric = fun (Base: Base) => ({
  ...
  include (TransientSequentialCollection.MakeGeneric Base: TransientSequentialCollection.SGeneric with type t 'a := t 'a and type elt 'a := elt 'a);
  ...
  let addLastAll (iter: Iterable.t (elt 'a)) (transient: t 'a): (t 'a) =>
    iter |> Iterable.reduce while_::Functions.alwaysTrue2 (fun acc next => acc |> addLast next) transient;
