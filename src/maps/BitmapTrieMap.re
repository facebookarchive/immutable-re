...
let rec containsKey =
        (comparator: Comparator.t('k), depth: int, hash: int, key: 'k, set: t('k, 'v))
        : bool =>
  switch set {
...
let rec get =
        (comparator: Comparator.t('k), depth: int, hash: int, key: 'k, map: t('k, 'v))
        : option('v) =>
  switch map {
  | Level(bitmap, nodes, _) =>
    let bit = BitmapTrie.bitPos(hash, depth);
    ...
    if (BitmapTrie.containsNode(bitmap, bit)) {
      get(comparator, depth + 1, hash, key, nodes[index])
    } else {
      None
    }
  | Collision(entryHash, entryMap) =>
    if (hash === entryHash) {
      AVLTreeMap.get(comparator, key, entryMap)
    } else {
      None
    }
...
let rec getOrDefault =
        (comparator: Comparator.t('k), depth: int, default: 'v, hash: int, key: 'k, map: t('k, 'v))
        : 'v =>
  switch map {
  | Level(bitmap, nodes, _) =>
    let bit = BitmapTrie.bitPos(hash, depth);
    let index = BitmapTrie.index(bitmap, bit);
    if (BitmapTrie.containsNode(bitmap, bit)) {
      getOrDefault(comparator, depth + 1, default, hash, key, nodes[index])
    } else {
      default
    }
  | Collision(entryHash, entryMap) =>
    if (hash === entryHash) {
      AVLTreeMap.getOrDefault(comparator, ~default, key, entryMap)
    } else {
      default
    }
  | Entry(entryHash, entryKey, entryValue) =>
...
let rec getOrRaise =
  ...
  | Level(bitmap, nodes, _) =>
    ...
    let index = BitmapTrie.index(bitmap, bit);
    if (BitmapTrie.containsNode(bitmap, bit)) {
      getOrRaise(comparator, depth + 1, hash, key, nodes[index])
    } else {
      failwith("NotFound")
    }
  | Collision(entryHash, entryMap) =>
    if (hash === entryHash) {
      AVLTreeMap.getOrRaise(comparator, key, entryMap)
    } else {
      failwith("NotFound")
    }
  | Entry(entryHash, entryKey, entryValue) =>
...
let rec putWithResult =
  ...
  | Collision(entryHash, entryMap) when hash === entryHash =>
    let newEntryMap =
      entryMap |> AVLTreeMap.putWithResult(comparator, alterResult, key, newEntryValue);
    switch alterResult^ {
    | AlterResult.Added =>
      alterResult := AlterResult.Added;
...
let rec alter =
  ...
  | Level(bitmap, nodes, _) =>
    ...
    if (BitmapTrie.containsNode(bitmap, bit)) {
      ...
      let newChildNode =
        childNode
        |> alter(comparator, updateLevelNode, owner, alterResult, depth + 1, hash, key, f);
      switch alterResult^ {
      | AlterResult.Added => map |> updateLevelNode(owner, index, newChildNode)
      | AlterResult.NoChange => map
      | AlterResult.Removed =>
        if (newChildNode === Empty) {
          ...
          updateLevelNode(owner, index, newChildNode, map)
        }
...
let updateLevelNodePersistent =
    (_: Transient.Owner.t, index: int, childNode: t('k, 'v), Level(bitmap, nodes, _): t('k, 'v))
    : t('k, 'v) =>
...
let updateLevelNodeTransient =
    (
      ...
      index: int,
      childNode: t('k, 'v),
      Level(bitmap, nodes, nodeOwner) as node: t('k, 'v)
    )
    : t('k, 'v) =>
  if (nodeOwner === owner) {
