...
let test = describe "Map" [
  ...
  it "equalsWith" (fun () => {
    ...
    expect (Vector.fromSeq @@ List.toSeq @@ [] |> Vector.toMap |> Map.equals Map.empty)
      |> toBeEqualToTrue;
    expect (Vector.fromSeq @@ List.toSeq @@ [1, 2, 3] |> Vector.toMap |> Map.equals Map.empty)
      |> toBeEqualToFalse;
    expect (Map.equals
      (Vector.fromSeq @@ List.toSeq @@ [1, 2, 3] |> Vector.toMap)
      (Vector.fromSeq @@ List.toSeq @@ [1, 2, 3] |> Vector.toMap)
    ) |> toBeEqualToTrue;
  }),
  it "hashWith" (fun () => {
  ...
  it "isEmpty" (fun () => {
    ...
    expect (Vector.fromSeq @@ List.toSeq @@ [0, 1, 2] |> Vector.toMap |> Map.isEmpty)
      |> toBeEqualToFalse;
  }),
  it "isNotEmpty" (fun () => {
    expect (Map.isNotEmpty Map.empty) |> toBeEqualToFalse;
    expect (Vector.fromSeq @@ List.toSeq @@ [0, 1, 2] |> Vector.toMap |> Map.isNotEmpty)
      |> toBeEqualToTrue;
  }),
  describe "keys" [
    it "contains" (fun () => {
