...
let test = describe "Map" [
  describe "empty"  [
    ...
    it "toSequence" (fun () => {
      expect (Map.empty |> Map.toSequence |> Sequence.isEmpty) |> toBeEqualToTrue;
    }),
    it "tryFind" (fun () => {
    ...
    it "values" (fun () => {
      expect (Map.empty |> Map.values |> Iterator.count) |> toBeEqualToInt 0;
    }),
  ],
  it "equalsWith" (fun () => {
    expect (Map.empty |> Map.equals Map.empty) |> toBeEqualToTrue;
    expect (Vector.from @@ List.toIterator @@ [] |> Vector.toMap |> Map.equals Map.empty)
      |> toBeEqualToTrue;
    expect (Vector.from @@ List.toIterator @@ [1, 2, 3] |> Vector.toMap |> Map.equals Map.empty)
      |> toBeEqualToFalse;
    expect (Map.equals
      (Vector.from @@ List.toIterator @@ [1, 2, 3] |> Vector.toMap)
      (Vector.from @@ List.toIterator @@ [1, 2, 3] |> Vector.toMap)
    ) |> toBeEqualToTrue;
  }),
  it "hashWith" (fun () => {
    expect (Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.hash)
      |> toBeEqualToInt (Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.hash)
  }),
  it "isEmpty" (fun () => {
    expect (Map.isEmpty Map.empty) |> toBeEqualToTrue;
    expect (Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap |> Map.isEmpty)
      |> toBeEqualToFalse;
  }),
  it "isNotEmpty" (fun () => {
    expect (Map.isNotEmpty Map.empty) |> toBeEqualToFalse;
    expect (Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap |> Map.isNotEmpty)
      |> toBeEqualToTrue;
  }),
  describe "keys" [
    it "contains" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
    ...
    it "toSequence" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let keys = map |> Map.keys;
      expect (keys |> Set.toSequence) |> toBeEqualToSequenceOfInt (List.toSequence [0, 1, 2]);
    }),
    it "tryFind" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let keys = map |> Map.keys;
  ...
  describe "map"  [
    ...
    it "toSequence" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let mapped = map |> Map.map (fun _ v => v + 3);
      expect (mapped |> Map.toSequence) |> toBeEqualToSequence (fun _ => "") (List.toSequence [(0, 3), (1, 4), (2, 5)]);
    }),
    it "tryFind" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let mapped = map |> Map.map (fun _ v => v + 3);
    ...
    it "values" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let mapped = map |> Map.map (fun _ v => v + 3);
      expect (mapped |> Map.values |> List.fromReversed |> List.toSequence)
        |> toBeEqualToSequenceOfInt (List.toSequence [5, 4, 3]);
    }),
  ],
  describe "toSet" [
    it "contains" (fun () => {
    ...
    it "toSequence" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let set = map |> Map.toSet;
      expect (set |> Set.toSequence) |> toBeEqualToSequence (fun _ => "") (List.toSequence [(0, 0), (1, 1), (2, 2)]);
    }),
    it "tryFind" (fun () => {
      let map = Vector.from @@ List.toIterator @@ [0, 1, 2] |> Vector.toMap;
      let set = map |> Map.toSet;
