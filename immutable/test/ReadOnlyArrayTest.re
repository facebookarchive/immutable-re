...
let test = describe "ReadOnlyArray" [
  ...
  it "map" (fun () => {
    ReadOnlyArray.ofUnsafe [| 1, 2, 3 |]
      |> ReadOnlyArray.map (fun i => i + 1)
      |> ReadOnlyArray.toIterator
      |> List.fromReverse
      |> Expect.toBeEqualToListOfInt [4, 3, 2];
  }),
  it "mapReverse" (fun () => {
    ReadOnlyArray.ofUnsafe [| 1, 2, 3 |]
      |> ReadOnlyArray.mapReverse (fun i => i + 1)
      |> ReadOnlyArray.toIterator
      |> List.fromReverse
      |> Expect.toBeEqualToListOfInt [2, 3, 4];
  }),
  it "mapWithIndex" (fun () => {
    ReadOnlyArray.ofUnsafe [| 0, 0, 0 |]
      |> ReadOnlyArray.mapWithIndex (fun i next => i + next + 1)
      |> ReadOnlyArray.toIterator
      |> List.fromReverse
      |> Expect.toBeEqualToListOfInt [3, 2, 1];
  }),
  it "mapReverseWithIndex" (fun () => {
    ReadOnlyArray.ofUnsafe [| 0, 0, 0 |]
      |> ReadOnlyArray.mapReverseWithIndex (fun i next => i + next + 1)
      |> ReadOnlyArray.toIterator
      |> List.fromReverse
      |> Expect.toBeEqualToListOfInt [1, 2, 3];
  }),
  it "ofUnsafe" (fun () => ()),
  it "reduce" (fun () => ()),
  it "reduceRight" (fun () => ()),
