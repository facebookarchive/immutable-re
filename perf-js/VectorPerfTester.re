...
let test (n: int) (count: int): Test.t => {
  ...
  let immJSVector = indexes
    |> IntRange.reduce (fun acc i => acc |> ImmJSRe.List.push i) immJSVectorEmpty;
  ...
  /*  describe "CamlMutableArray" (
    ...
    describe "Vector.Transient" (
      generateTests
        ...
        Vector.Transient.getOrRaise
        count
    ),
    describe "immJSVector" (
      generateTests
        (fun () => immJSVector)
        (fun () => immJSVectorEmpty)
        ImmJSRe.List.push
        ImmJSRe.List.set
        ImmJSRe.List.pop
        ImmJSRe.List.get
